...
export default async function EditAssignmentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: myGroupsData } = await supabase.from("groups").select("*").eq("teacher_id", profile!.id);
  const myGroups = (myGroupsData ?? []) as Group[];
  ...
  return (
    <div className="space-y-7">
      ...
      <AssignmentForm
        ...
        availableGroups={myGroups.map((g) => ({ id: g.id, name: g.name, emoji: g.emoji }))}
